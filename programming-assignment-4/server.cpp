...
int main (int argc, char* argv[]) {
	...
	// FIXME: open socket with address="" and r from CLIENT
	//		  then enter infinite loop calling TCPReqChan::accept_conn() and 2nd TCP constructor
	//		  dispatching handle_process_loop thread for new channel (create a thread then detatch the thread, look at other server PAs)

	string address = "";
	TCPRequestChannel* control_channel = new TCPRequestChannel(address, r);
	while (true) {

		/* 
		accept new connection socket
		create a new channel with the accepted socket (the scoket is alr established)
		dispatch a new thread to handle process loop
		since main tread is not blocked, it can accept more connections (use detatch instead of join)
		 */
		
		int sock = control_channel->accept_conn();
        if (sock < 0) {
            cerr << "Error accepting connection" << endl;
            break;
        }

        // Create a new channel for the accepted connection
        TCPRequestChannel *new_channel = new TCPRequestChannel(sock);
        
        // Dispatch a thread to handle the new channel
        thread new_thread(handle_process_loop, new_channel);
        new_thread.detach(); // Detach the thread to run independently
	}
